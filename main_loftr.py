...
if __name__ == '__main__':
    ...
    for i in range(1, len(proj_images)):
        ...
        img_prev_gray = cv.cvtColor(img_prev, cv.COLOR_BGR2GRAY)
        img_curr_gray = cv.cvtColor(img_curr, cv.COLOR_BGR2GRAY)
        
        scale_prev = 640.0 / max(img_prev_gray.shape)
        scale_curr = 640.0 / max(img_curr_gray.shape)
        
        img_prev_small = cv.resize(img_prev_gray, (0, 0), fx=scale_prev, fy=scale_prev)
        img_curr_small = cv.resize(img_curr_gray, (0, 0), fx=scale_curr, fy=scale_curr)
        
        import kornia as K
        import torch
        tensor_prev = K.image_to_tensor(img_prev_small, False).float() / 255.
        tensor_curr = K.image_to_tensor(img_curr_small, False).float() / 255.
        tensor_prev = tensor_prev.to(device)
        tensor_curr = tensor_curr.to(device)
        
        input_dict = {"image0": tensor_prev, "image1": tensor_curr}
        ...
        pts1 = mkpts0 / scale_prev
        pts2 = mkpts1 / scale_curr
    ...
    for i in range(1, len(proj_images)):
        ...
        pts1, pts2 = matches_list[i-1]
        inlier_mask = inlier_masks[i-1]
        img_matched = draw_matches_custom(img_prev, img_curr, pts1, pts2, mask=inlier_mask)
        row2 = img_matched
